...
//            stmt.execute("insert into MyAdministrator(admin_inum) value('1234567890')");
            main_Mode();
//            rs.close();
            stmt.close();
            con.close();
    ...
    public static void search_for_User() {
        ...
        try {
            ...
            String browse_for_watching_query = "select * from myuser, myvideo, mywatchinglist where myuser.u_inum = ? and myuser.u_inum = mywatchinglist.u_inum and mywatchinglist.v_inum = myvideo.v_inum";
            String browse_for_upload_query = "select * from myuser, myvideo where u_inum = ? and u_inum = uploader_u_inum";
            stmt = con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
//            pstmt = con.prepareStatement(search_for_user_query, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
//            pstmt.setString(1, search);
    ...
    public static void user_Create() {
        String u_id;
        String u_password;
        // 이 while문이 끝나면 create 끝 -> log_in 창으로 넘어가자.
        while(true) {
            try {
                ...
                // 일단 암호화 안된 id, password 찾아서 되면은 암호,복호화 해보자.
//                        String user_insert_encrypt_query = "insert into MyUser (u_inum, u_id, hex(aes_encrypt(u_password, u_id)), u_channelName) values (?, ?, ?, ?)";
...
//                showdefaultMenu("User");
                ...
                if(rs.next()) {
                    ...
                    // ID 비번 일치하면, while break.
                    // ID 비번 일치완료. -> 해당 user의 시점에서 활동. -> user method 시작.
                    // user method의 인자는 u_inum, u_channelName?

                    user_Actions(rs.getString("u_inum"), rs.getString("u_channelName"));
                    // user method 끝나면, user_Login method end.
                    return;
                }
    ...
    public static void user_Actions(String u_inum, String u_channelName) {
        try {
            while(true) {
                ...
                System.out.println("4. About subscribee / subscriber");
                System.out.println("5. User info");
                System.out.println("6. Administrate vidoes");
                System.out.println(underbars);
                menu = scanner.nextInt();
                if(menu == 0) {
                    System.out.println("Go back to previous menu..");
    ...
    public static void administrate_user_Videos(String u_inum) {
        ...
        // 결국엔 showvideoresults로. mode 사용해서.
    }
    ...
    public static void upload_Videos(String uploader_u_inum, String uploader_channelName) {
        try {
            ...
            while(true) {
                ...
                // "Game", "Movie", "Sports", "News", "Music", "Economics", "Education", "Etc"
                ...
                if(menu == 1) {
                    // insert.
                    // v_title, v_length, v_genre, isAdver
    ...
    public static void watch_Videos(String u_inum) {
        ...
        while(true) {
            ...
            // 시청할 때 마다 u_inum에 해당하는 시청내역에 포함시키기.
            if(menu == 0) {
                System.out.println("Go back to previous menu..");
                return;
            }
    ...
    public static void showvideoResults(ResultSet rs, String u_inum, int mode) {
        ...
        try {
            ...
            while(rs.next()) {
                ...
                ResultSet tmp = pstmt.executeQuery();
//                channelName = tmp.getString("u_channelName");
                tmp.next();
    ...
    public static void watch_Video(String v_inum, String u_inum) {
        try {
            ...
            rs = pstmt.executeQuery();
            // 어차피 해당 video 하나만의 결과만 볼 거니까 바로 next.
            rs.next();
    ...
    public static void search_Videos(String u_inum) {
        ...
        while(true) {
            ...
            switch (menu) {
                case 1 :
                    try {
                        // v_title, v_length, v_count, v_like, v_uploadDate, v_genre, isAdver
//                        String search_with_title_query = "select v_inum, v_title, v_length, v_count, v_like, v_uploadDate, v_genre, isAdver from myvideo where v_title like ?";
                        String search_with_title_query = "select * from myvideo where v_title like ?";
...
//                channelName = tmp.getString("u_channelName");
    ...
    // 그냥 구독목록만 출력하기 위함. watch는 따로 x.s
    public static void show_Subscribees(String u_inum, String u_channelName) {
        ...
        String get_channelName_query = "select * from myuser where u_inum = ?";
